        ...
        for lamda3 = lamda3s
            ...
            try
                for fold_num = 1:folds_num              
                    ...
                    te_Y = vec2lab(te_Y);
                    [ acc, pre, rec, f1 ] = confusion_matrix(te_label, te_Y);
                    result(fold_num,1) = acc;
                    result(fold_num,2) = pre;
                    result(fold_num,3) = rec;
                    result(fold_num,4) = f1;
                end
            catch err
                disp(err);
            ...
            if (acc_te_mean>best_acc_te)
                ...
                best_acc_result.pre_mean = pre_te_mean;
                best_acc_result.rec_mean = rec_te_mean;  
                best_acc_result.f1_mean = f1_te_mean;  
                best_acc_result.lamda_scale = best_lamada_scale;
            end
            fprintf('train mul TSK FS:%d/5\nNumber of iterations:%d-----%d-----%d\n', fold, a, b, c);
            fprintf('best acc result:\nacc:%.4f  pre:%.4f  recall:%.4f  f1:%.4f\n',best_acc_result.acc_mean, best_acc_result.pre_mean, best_acc_result.rec_mean, best_acc_result.f1_mean);
        end %end lamda3s
    end %end lamda2s
end %end lamda1s
